...
export default function Page() {
  ...
  const cards = [
    ...
    {
      title: 'Entrate',
      value: `€${((kpis.incomeCents ?? 0) / 100).toFixed(2)}`,
    },
    {
      title: 'Associazioni',
      value: kpis.associationsCount ?? 0,
    },
  ];
  ...
  const maxRevenue = Math.max(
    ...chartData.map((item) => item.revenue || 0),
    1,
  );

  return (
    <main className="min-h-screen bg-[#090D14] p-8 text-white">
      <div className="mb-10">
        ...
        <p className="mt-4 max-w-2xl text-zinc-400">
          Dati reali collegati al backend NestJS.
        </p>
      </div>
